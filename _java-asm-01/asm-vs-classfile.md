...
刚刚我们简单介绍了Class File Format的概念，它就是`.class`文件所遵循的数据结构。现在我们来介绍“字节码的类库”，那么Class File Format和字节码的类库之间是什么样的关系呢？我们可以用下图来表示：
...
- 中下层－不论这些`.class`文件内容有怎样大的差异，它们都共同遵守同一个数据结构，即Class File Format。
